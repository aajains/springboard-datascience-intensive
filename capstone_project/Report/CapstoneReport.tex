...
\begin{table}[!h]
\centering
\caption{Comparing all models: Red for the worst and green for the best model.}\label{tab:modelcomp}
\scalebox{1}{
...
\end{table}
AUC for both ROC and PR are much better for all models, however, the ET model again performs the best with PR AUC = 0.91 and ROC AUC = 0.89. Corresponding ROC and PR curves are shown in Fig. \ref{fig:AllModelsUndersampledTest}. 
\begin{figure}[!h]
\begin{center}
\includegraphics[width=6in]{UndersampledTestDataAllModels_ROC_PR_plots.pdf}
\end{center}
\caption{\label{fig:AllModelsUndersampledTest}
Comparing ROC and PR curves for all models when test data is under-sampled. LR: Logistic Regression, GNB: Gaussian Naive Bayes, RF: Random Forest, GB: Gradient Boosting, ET: Extremely Randomized Trees.}
\end{figure}
Using under-sampled test data to calculate all performance metrics, we obtained much better values for PR and ROC AUC but the best model is ET model only. 
%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Using Model and Recommendations}
\label{sec:reco}
%%%%%%%%%%%%%%%%%%%%%%%%%
We now briefly discuss about using the best model to predict flight cancellation likelihood. To make predictions on new data, one would need to filter out some features and then perform OHE on categorical features. Other than these feature processing steps, all other pre-processing steps are stored in the model pipeline. Table \ref{tab:finalfeatures} contains the list of all 31 features that we need to feed to the ET classification model. The table is sorted by the feature importance values obtained from the ET model, with highest on the top.
\begin{table}[!h]
\centering
\caption{Final set of features for the ET model}\label{tab:finalfeatures}
\scalebox{0.85}{
\begin{tabular}{lccclccc}
\toprule
\textbf{Features} & \textbf{Source type} & \textbf{Data type} & \textbf{OHE required?} & \textbf{Description}  \\
\midrule
Month\_Dest  & Flight data & Categorical & Yes & \pbox{10cm}{Month at destination}  \\ 
\addlinespace
Month  & Flight data & Categorical & Yes & \pbox{10cm}{Month at origin}  \\ 
\addlinespace
Humidity\_Origin  & Weather data & Numerical & No & \pbox{10cm}{Humidity at origin ($\%$)}   \\ 
\addlinespace
Humidity\_Dest  & Weather data & Numerical & No & \pbox{10cm}{Humidity at destination ($\%$)}  \\ 
\addlinespace
Ncan\_30  &Flight history data & Numerical & No & \pbox{10cm}{Number of cancellations \\ in last 30 days}  \\ 
\addlinespace
Temperature\_Dest  & Weather data & Numerical & No & \pbox{10cm}{Temperature at destination ($^\circ$F)}  \\ 
\addlinespace
Carrier  & Flight data & Categorical & Yes & Airline carrier \\
\addlinespace
DayOfMonth\_Dest  & Flight data & Categorical & Yes & \pbox{10cm}{Day of month \\ at destination}  \\ 
\addlinespace
Temperature\_Origin  & Weather data & Numerical & No & \pbox{10cm}{Temperature at origin ($^\circ$F)}  \\ 
\addlinespace
...
We now discuss little bit about recommending some ideas to potential clients. Since we are only interested in predicting the probabilities of both negative and positive classes, we do not bother about the choice of thresholds or operating point in Figs \ref{fig:AllModels} and \ref{fig:AllModelsUndersampledTest}. The ET model would give us the probabilities of both classes. These probability values can then be used to make categories to indicate different levels of warning or alert messages. For example, if p(cancelled) is the probability of a flight being cancelled, then we can have a categorization like the one shown in Tab. \ref{tab:alertexample}.  
\begin{table}[!h]
\centering
\caption{An example of a possible categorization of alerts messages to indicate flight cancellation chances}\label{tab:alertexample}
\scalebox{1.0}{
\begin{tabular}{lcccccc}
\toprule
\textbf{Probability ranges} & \textbf{Alert messages}  \\
\midrule
\rc p(cancelled)  > 0.9 & \rc  Extremely high chance  \\ 
\oc 0.75 < p(cancelled) $\leq$ 0.9 & \oc Very high chance \\ 
\yoc 0.5 < p(cancelled) $\leq$ 0.75 & \yoc High chance \\ 
\yc 0.25 < p(cancelled) $\leq$ 0.5 & \yc Moderate chance \\ 
\sgc 0.1 < p(cancelled) $\leq$ 0.25 & \sgc Low chance  \\
\gc p(cancelled) $\leq$ 0.1 & \gc Very low chance \\  
\bottomrule \\
\end{tabular}
}
\end{table}
We can recommend such a categorization to different clients, whether it is a booking website or for an app, depending on their business model and requirements. 
%%%%%%%%%%%%%%%%%%%%%%%%%
